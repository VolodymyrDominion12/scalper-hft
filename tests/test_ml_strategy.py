    ...
    def test_no_lookahead_in_train(self):
        ...
        for train_idx, test_idx in pkf.split(X, t1=t1):
            ...
            assert len(violators) == 0, \
                f"Lookahead: {len(violators)} лівих зразків мають t1 > test_start"
...
class TestMLStrategy:
    ...
    def test_signals_are_valid_values(self):
        ...
        assert set(signals.unique()).issubset({-1, 0, 1}), \
            f"Невалідні значення: {signals.unique()}"
    ...
    def test_vol_regime_filter(self):
        ...
        strat = get_strategy(
            "ml_strategy",
            train_bars=50, test_bars=20, holding_bars=8,
            regime_filter="vol", vol_regime_ok="normal",
        )
        signals = strat.generate_signals(df)
        assert signals.index.equals(df.index)
