...
filtered_accounts = accounts_df[
    (accounts_df["exchange"].isin(selected_exchanges)) & (accounts_df["mode"].isin(selected_modes))
]
...
if bots_df.empty:
    ...
    filtered_bots = bots_df[(bots_df["exchange"].isin(selected_exchanges)) & (bots_df["mode"].isin(selected_modes))]
    st.dataframe(filtered_bots, width="stretch", hide_index=True)
