...
## 1. Мета
MFT квант-платформа для Binance USDT-M ф'ючерсів (хвилини–години; не субмілісекундний HFT):
цикл **дослідження → реалізація → тестування → аудит → покращення** з жорстким
захистом від перенавчання. Валідований кандидат — `pairs_arb` LINK/BTC 1h maker.
Retail-латентність з дому 50–200 мс достатня для 1h maker; tick-MM потребує L2/colo
(див. [docs/RESEARCH.md](RESEARCH.md), [docs/analysis_hft_2026.md](analysis_hft_2026.md)).
