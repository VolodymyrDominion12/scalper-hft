...
> **Статус виправлень:** план і прогрес — `docs/reports/fix_plan_2026-09-11.md`.
> Станом на 2026-09-11 **P0 закрито повністю** (K1, K2, K4, K5, K6 + 3 нові знахідки:
> тихий нуль `basis_reversion`, невідтворюваний Exp3, `flat_sharpe=0.0`), 37 нових тестів,
> `pytest` → 1471 passed / 0 failed, артефакти iter7 перегенеровано. Старі артефакти —
> `results/archive_pre_audit_2026-09-11/`. Числа в розділах K1/K2 нижче — **до** виправлень
> (саме вони й були дефектом); актуальні — у плані.
...
**Уточнення механізму (детальніший розбір).** Я розклав втрату за довжиною інтервалу між
сусідніми збереженими угодами:

| інтервал між сусідніми рядками | пар | медіанний стрибок `trade_id` | втрачено id |
|---|---|---|---|
| ≤ 1 мс | 106 295 | 3.0 | 612 912 |
| 2–10 мс | 219 425 | 1.0 | 594 005 |
| 11 мс – 1 с | 815 327 | 1.0 | 591 907 |
| > 1 с | 32 504 | 1.0 | 11 118 |

Отже 78.5% сусідніх пар мають `Δ trade_id == 1` (потік переважно неперервний), і втрата
концентрується у «сплесках». Дедуп за мілісекундним індексом **доведено** винний у першому
рядку таблиці (спільна мс → лишається одна угода; саме так зникають 613 тис. id). Але
рядки 2–3 (1.19 млн id при Δt > 1 мс) дедуп за мс пояснити **не може** — там працює друга
причина, на рівні самого запиту: ccxt для лінійних фʼючерсів ставить
`endTime = since + 1 год`, а `limit` обрізаний до 1000, тоді як BTCUSDT perp дає десятки
тисяч aggTrades на годину. Курсор `since = last_ts + 1` довантажує далі, але частина вікна
лишається не вибраною.

**Тому виправлення точкове і не покладається на один механізм:** дедуп за `trade_id`
(прибирає першу причину) **і** пагінація за `fromId` замість часу (прибирає другу). Обидва
внесено; виміряно на синтетиці з реальним профілем (5 угод/мс): стара логіка лишала
500 із 2500 угод, нова — усі 2500.
