...
def load_sweep(mode: str) -> pd.DataFrame:
    ...
    df = pd.read_sql_query("SELECT * FROM sweep_results WHERE days=? AND mode=?", con, params=(DAYS, mode))
    con.close()
    return df
                    ...
                    [
                        {
                            ...
                            "oos_pos_frac": float(r["oos_positive_frac"]),
                        }
...
def stage_audit() -> pd.DataFrame:
    ...
    with ProcessPoolExecutor(max_workers=6) as ex:
        ...
        for i, fut in enumerate(as_completed(futs), 1):
            ...
            rows.append(r)
            print(
                f"[{i}/{len(tasks)}] {r.get('symbol')} {r.get('interval')} "
                f"{r.get('strategy')} status={r.get('status')}",
                flush=True,
            )
...
def stage_report() -> str:
    ...
    lines.append("")
    lines.append(
        "Параметри: 180 днів (2026-03-08 → 2026-09-04), 15 символів USDT-M, 6 таймфреймів, "
    ...
    if not ok_wf.empty:
        ...
        agg = (
            ...
            .sort_values("mean_oos", ascending=False)
            .reset_index()
        )
    ...
    for _, r in winners.sort_values(["interval", "symbol"]).iterrows():
        lines.append(
            f"| {r['symbol']} | {r['interval']} | {r['winner_strategy']} | "
    ...
    if not audits.empty:
        ...
        lines.append("")
        lines.append(
            "| символ | ТФ | стратегія | вердикт | avg_OOS | OOS>0 | DSR | smooth | trades | ret | Sharpe | причина |"
        )
        lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|")
        for _, r in audits.sort_values(["interval", "symbol"]).iterrows():
            ...
            sh = r.get("bt_sharpe")
            fmt = lambda x, d=2, suf="": (
                f"{x:+.3f}"
                if suf == "s"
                else f"{x:+.2%}"
                if suf == "pct"
                else (f"{x:.0f}" if x is not None and pd.notna(x) else "-")
            )
            lines.append(
    ...
    lines.append(
        "avg OOS Sharpe > 0.30; частка OOS-вікон > 0 ≥ 50%; DSR > 0.95; smoothness > 0.30; "
